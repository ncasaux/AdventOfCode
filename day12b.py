...
for p in startingPoints:
    itineraryWithPoints = {p: 1}
    new_list = findPathsToExitFromPoint(p)
    ...
    while not found_or_stuck:
        if endPoint not in new_list:
            ...
            steps.append(itineraryWithPoints[endPoint] - 1)
            print(f"Exit found in {itineraryWithPoints[endPoint] - 1} steps.")
            found_or_stuck = True

print(f"Minimum number of steps to found exit: {min(steps)}.")
